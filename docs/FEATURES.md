...
- **Chevron kamera dihapus dari control bar**: kamera jadi toggle polos; opsi kamera/background hanya di Settings → Video (menu ⋯ → Backgrounds and effects).

### Room UI overhaul — Google Meet-style
- **ControlBar** ditulis ulang mengikuti layout Meet: mic · kamera · layar · reaksi · captions · angkat tangan · `⋮` More options (menu tunggal) · end call. Rail vertikal kanan: **Chat** (badge unread) + **People**. Semua chrome gelap permanen (independen dari tema).
